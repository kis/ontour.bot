...
async function startAnalysis(userId) {
    try {
        ...
        const artists = lodash_1.default.chain(data.Items)
            ...
            .uniqWith((a, b) => { return lodash_1.default.lowerCase(a) == lodash_1.default.lowerCase(b); })
            .value();
        console.log("Searched artists", artists);
        const cities = lodash_1.default.chain(data.Items)
            .sortBy((item) => item.params)
