...
async function fetchLocation(location_query) {
    return request
        ...
        .query({ apikey: config.SONGKICK_API_KEY, query: location_query, per_page: constants.ITEMS_PER_PAGE });
}
...
async function fetchEventsByMetroAreaID(id, fromDate, toDate, page) {
    return request
        ...
        .query({ apikey: config.SONGKICK_API_KEY, per_page: constants.EVENTS_PER_PAGE, page: page })
...
async function fetchArtist(artist_name) {
    return request
        ...
        .query({ apikey: config.SONGKICK_API_KEY, query: artist_name, per_page: constants.ITEMS_PER_PAGE });
}
