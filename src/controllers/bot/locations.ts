...
import { fetchLocationsByCoords } from '../../api';
...
bot.onText(/\/locations/, async (msg: any) => {
  ...
  try {
    ...
    bot.sendMessage(
      msg.chat.id,
      getLanguage().LOCATION_NOT_FOUND,
...
function setSearchParams(
  ...
  bot.sendMessage(
    chatID,
    getLanguage().LOCATION_SEARCH(city.displayName),
    constantsReply.REPLY_OPTIONS
  );
  setLocationSearchParams({
    metroAreaID,
...
async function getNextEventsByMetroAreaID() {
  ...
  const { eventsList, eventsCount } = await getEventsByMetroAreaID(
    metroAreaID,
    fromDate,
    toDate,
    searchPage
  ...
  const message = !eventsCount
    ? getLanguage().EVENTS_NOT_FOUND
    : getLanguage().EVENTS_FOUND(eventsCount);
...
bot.on('message', async (msg: any) => {
  ...
  if (msg.text === getLanguage().NEXT) {
    if (
      getLocationSearchParams() &&
      getSearchType() === constantsSearch.LOCATIONS_SEARCH
    ) {
      await getNextEventsByMetroAreaID();
    }
  }
