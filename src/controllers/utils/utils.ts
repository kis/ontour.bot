...
function getCitiesTemplate(citiesList: Array<any>) {
  ...
  return citiesList.reduce((acc: string, city: any) => {
    return (
      acc +
      `<b>${city.city.displayName}</b>, ${city.city.country.displayName}\n\n`
    );
  }, citiesTpl);
}
...
async function getEventsByArtist(
  artist: any,
  fromDate: any,
  toDate: any,
  page: number
...
async function getEventsByMetroAreaID(
  metroAreaID: number,
  fromDate: any,
  toDate: any,
  page: number
) {
  let events = await fetchEventsByMetroAreaID(
    metroAreaID,
    fromDate,
    toDate,
    page
