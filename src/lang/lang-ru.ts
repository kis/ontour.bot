...
const BAND_SEARCH = (band: string) => `А теперь я поищу туры и концерты по группе ${band}...`;
...
const LOCATION_SEARCH = (location: string) => `А теперь я поищу концерты в городе ${location}...`;
...
const EVENTS_FOUND = (count: number) => `Я нашел ${count} концертов, я буду выдавать по 5 концертов в сообщении, чтобы перейти к следующим жми Далее`;
