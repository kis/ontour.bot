const WELCOME = (name: string) => `<b>Привет, ${name}!</b> 😄😉 \n\n
...
const DATE_COMMANDS = `<b>Для поиска концерта по датам:</b> 💬
...
Или нажми Next для вывода всех концертов`;

const SET_LANG = 'Выбери язык ;)';

const LANG_IS = (lang: string) => `Установлен ${lang} язык. Нажми /start`;

const BAND = 'Круто, теперь введи название группы';

const BAND_SEARCH = (band: string) =>
  `А теперь я поищу туры и концерты по группе ${band}...`;

const BAND_NOT_FOUND =
  'Извини, по этой группе я ничего не нашел :( Нажми /start для нового поиска';

const LOCATION =
  'Круто, теперь введи название города, в котором будем искать концерты';

const MY_LOCATION = 'Круто, а в каком городе ты находишься?';

const LOCATION_SEARCH = (location: string) =>
  `А теперь я поищу концерты в городе ${location}...`;

const LOCATION_NOT_FOUND =
  'Извини, в этом городе я не нашел концертов :( Нажми /start для нового поиска';

const DATES_FROM =
  'Пожалуйста, введи дату (в формате YYYY-MM-DD), с которой начать поиск концертов, или нажми Далее';

const DATES_TO =
  'Пожалуйста, теперь введи дату (в формате YYYY-MM-DD), до которой начать поиск концертов, или нажми Далее';

const EVENTS_NOT_FOUND =
  'Извини, похоже концертов нет :( Нажми /start для нового поиска';

const EVENTS_FOUND = (count: number) =>
  `Я нашел ${count} концертов, я буду выдавать по 5 концертов в сообщении, чтобы перейти к следующим жми Далее`;
