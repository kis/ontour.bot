...
async function getDates(datesRes, msg) {
  ...
  switch (datesRes) {
    ...
    case '/next_month':
      ...
      break;
    case '/enter_dates':
      fromDate = await askDate(
        msg.chat.id,
        instance_1.getLanguage().DATES_FROM
      );
...
async function askDate(chatId, date) {
  ...
  return await new Promise((resolve, reject) => {
    bot_1.default.once('message', async (reply) => {
      if (reply.text == instance_1.getLanguage().NEXT) resolve(null);
      resolve(reply.text);
    });
  });
}
function sendMessageWithNext(chatID, message) {
  bot_1.default.sendMessage(
    chatID,
    message,
    constantsReply.KEYBOARD_NEXT_OPTIONS(instance_1.getLanguage())
  );
}
