...
bot_1.default.onText(/\/artists/, async (msg) => {
  ...
  try {
    ...
    bot_1.default.sendMessage(
      msg.chat.id,
      instance_1.getLanguage().BAND_NOT_FOUND,
...
async function askArtist(chatId) {
  ...
  return await new Promise((resolve, reject) => {
    bot_1.default.once('message', async (reply) => {
      await logger_1.log(
        reply,
        constantsEvents.EVENT_ARTIST_SEARCH,
        reply.text
      );
...
async function setSearchParams(artists, fromDate, toDate, chatID) {
  ...
  bot_1.default.sendMessage(
    chatID,
    instance_1.getLanguage().BAND_SEARCH(artist.displayName),
    constantsReply.REPLY_OPTIONS
  );
...
async function getNextEventsByArtist() {
  ...
  const { eventsList, eventsCount } = await utils_1.getEventsByArtist(
    artist,
    fromDate,
    toDate,
    searchPage
  );
  const similarTpl = await utils_1.getSimilarArtistsTemplate(
    artist.displayName
  );
  let message;
  if (!eventsCount) {
...
bot_1.default.on('message', async (msg) => {
  ...
  if (msg.text === instance_1.getLanguage().NEXT) {
    if (
      params_1.getArtistSearchParams() &&
      params_1.getSearchType() === constantsSearch.ARTISTS_SEARCH
    ) {
      await getNextEventsByArtist();
    }
  }
