...
bot_1.default.onText(/\/locations/, async (msg) => {
    ...
    try {
        ...
        bot_1.default.sendMessage(msg.chat.id, instance_1.getLanguage().LOCATION_NOT_FOUND, constantsReply.REPLY_OPTIONS);
    }
});
async function askLocation(chatId) {
    bot_1.default.sendMessage(chatId, instance_1.getLanguage().LOCATION, constantsReply.REPLY_OPTIONS);
    return await new Promise((resolve, reject) => {
        bot_1.default.once("message", async (reply) => {
            await logger_1.log(reply, constantsEvents.EVENT_LOCATION_SEARCH, reply.text);
...
function setSearchParams(cities, fromDate, toDate, chatID) {
    ...
    bot_1.default.sendMessage(chatID, instance_1.getLanguage().LOCATION_SEARCH(city.displayName), constantsReply.REPLY_OPTIONS);
...
async function getNextEventsByMetroAreaID() {
    const { metroAreaID, city, fromDate, toDate, chatID } = params_1.getLocationSearchParams();
    let searchPage = params_1.getSearchPage();
    searchPage++;
    params_1.setSearchPage(searchPage);
    const { eventsList, eventsCount } = await utils_1.getEventsByMetroAreaID(metroAreaID, fromDate, toDate, searchPage);
    const message = !eventsCount ? instance_1.getLanguage().EVENTS_NOT_FOUND : instance_1.getLanguage().EVENTS_FOUND(eventsCount);
    bot_1.default.sendMessage(chatID, message, constantsReply.REPLY_OPTIONS);
    if (!eventsList.event || !eventsList.event.length)
        return;
    let eventTpl = utils_1.getEventsListTemplate(eventsList, city, constantsSearch.LOCATIONS_SEARCH);
...
bot_1.default.on('message', async (msg) => {
    ...
    if (msg.text === instance_1.getLanguage().NEXT) {
        if (params_1.getLocationSearchParams() && params_1.getSearchType() === constantsSearch.LOCATIONS_SEARCH) {
            await getNextEventsByMetroAreaID();
        }
    }
});
bot_1.default.onText(/\/mylocation/, async (msg) => {
    ...
    bot_1.default.sendMessage(msg.chat.id, utils_1.getCitiesTemplate(citiesArray), constantsReply.REPLY_OPTIONS);
});
async function askMyLocation(chatId) {
    bot_1.default.sendMessage(chatId, instance_1.getLanguage().MY_LOCATION, constantsReply.REPLY_LOCATION);
    return await new Promise((resolve, reject) => {
        bot_1.default.once("location", async (reply) => {
            await logger_1.log(reply, constantsEvents.EVENT_MY_LOCATION, reply.location);
            bot_1.default.sendMessage(reply.chat.id, "Your location is " + [reply.location.longitude, reply.location.latitude].join(";"));
